...
Worker-only (panel calls return `panel_forbidden`): `storage.*`, `secrets.*`,
`events.subscribe`. Those stay out of the panel surface so an XSS'd iframe
cannot dump the plugin vault or subscribe to host events.

Mixed versions: an older host still answers `panel_forbidden` for settings.
A newer host accepting settings over the existing `plugins:panelAction` /
`plugins.hostCall.panel` RPCs is additive — no new stream opcode.
...
5. A second plugin's panel cannot read or write this file; storage actions
   from the panel still fail with `panel_forbidden`.
